...
function App() {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8001/api/properties/")
      .then(res => {
  ...
  const buy = async (id) => {
    await axios.post("http://127.0.0.1:8001/api/buy/", { property_id: id, tokens: 1 })
    alert("🎉 1 Token Bought Successfully!")
  }
        ...
        {loading ? "Loading..." : props.map(p => (
          <div key={p.id} style={{ background:'white', borderRadius:16, overflow:'hidden', boxShadow:'0 10px 30px rgba(0,0,0,0.08)', border:'1px solid #e2e8f0' }}>
            ...
            <div style={{ padding:24 }}>
              ...
              {/*<div style={{ fontSize:12, background:'#fef3c7', color:'#92400e', display:'inline-block', padding:'4px 8px', borderRadius:6, fontWeight:600, marginBottom:12 }}>KOKAPET • HYDERABAD</div>*/}
